...
from module1.preprocessing import filter
import pywt
    ...
    for ds in downsampling_levels:
        ...
        for data, movement_type in zip([data_1, data_2], [type_of_movement_1, type_of_movement_2]):
            ...
            for i, epoch_data in enumerate(X):
                ...
                filtered_data = filter(epoch_data, wave_type, samplerate)  # バンドパスフィルタリング
                all_filtered_data.append(filtered_data)  # バンドパスフィルタリングしたデータをリストに追加

            all_filtered_data = np.array(all_filtered_data)  # リストをnumpy配列に変換
            all_filtered_data = np.array(all_filtered_data).transpose(0,2,1) # 形状を(試行数,サンプル点数,チャンネル数)に

            # 運動状態ごとのデータとラベルを辞書に追加
            all_data_dict[movement_type] = {"epoch_data": all_filtered_data, "labels": y}
        # save the data into a .npy file
        save_dir = current_dir / "BPF_data" / ext_sec / f"ds_{ds}" / subject_id
        os.makedirs(save_dir, exist_ok=True)
        output_file = save_dir / "bpf_data.npy"
